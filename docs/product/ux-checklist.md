...
**UX-детали:**
- В welcome явно: «Чтобы попасть в группу в приложении, откройте Skinemsya из этого чата».
- Команды бота: `/start` (приветствие), `/help` (справка), `/open` (кнопка приложения). Без личных уведомлений.
...
- [ ] Если все обычные позиции разобрали и нет общих, на экране выбора остаётся список и кнопка «Готово»; выбор также завершается автоматически. Статус в списке участников — «Готово», не «Ещё не зашёл». Кнопка ↻ на выборе не крутится от тихого обновления раз в 3 с.
...
- Список участников со статусами: **Выбирает** / **Готово** (нулевой выбор) / **Не скинул** / **Ждёт проверки** / **Подтверждено**.
