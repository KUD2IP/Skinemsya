...
- Кнопки: **«Сфоткать чек»** / **«Добавить вручную»**.
...
- Кнопка **«Скинуть {amount} ₽»** → экран реквизитов.

**Экран «Реквизиты»:**
- Сумма крупным шрифтом.
- Кнопка **«Скопировать»** реквизиты (один тап).
- Подсказка: «Переведи в банке, вернись сюда».
- Обязательное поле: **прикрепить скрин перевода**.
- Кнопка **«Отправил»** (неактивна без скрина).

**Acceptance criteria:**
- [ ] Нельзя выбрать qty больше доступного.
- [ ] Нельзя нажать «Скинуть» без выбора хотя бы одного блюда.
- [ ] «Отправил» требует скрин перевода.
...
- Один участник в группе → «Пригласи друзей в чат».
- Плательщик = единственный участник → запуск сбора заблокирован.
...
- [ ] Загрузка изображения чека (`files`).
- [ ] Интеграция ML-сервиса, парсинг позиций.
- [ ] Ручное добавление/редактирование позиций.
- [ ] Цена за штуку при qty > 1.
- [ ] Автоопределение и разделение чаевых.
- [ ] Пометка «На всех» (общая позиция).
- [ ] `sendToDistribution` → статус DISTRIBUTION.
- [ ] Блокировка редактирования после запуска.

**Phase 5 — Debts & Payments:**
- [ ] Выбор блюд участниками с ограничением qty.
- [ ] Расчёт долгов после выбора всех.
- [ ] Экран реквизитов с копированием.
- [ ] Прикрепление скрина перевода к Payment.
- [ ] «Отправил» → `debtor_confirmed`.
- [ ] «Всё на месте» → bulk `payer_confirmed`.
- [ ] «Не пришло» → `disputed` + уведомление.
- [ ] Экран статусов для плательщика.
- [ ] Автозакрытие сбора при всех подтверждённых.

**Phase 6 — Polish & Notifications:**
- [ ] Deep link `event_{eventId}`.
- [ ] Уведомление в группу при запуске сбора.
- [ ] Уведомление в группу при «Отправил».
- [ ] Уведомление в группу при закрытии сбора.
- [ ] DM плательщику при новом «Отправил».
- [ ] DM должнику при «Не пришло».
- [ ] «Напомнить» не скинувшим / не выбравшим.
- [ ] Отложенное напоминание плательщику (2 ч).
- [ ] Главный экран со сводкой долгов.
